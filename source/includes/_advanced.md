...
```  
To speed up flashing, you can run `./chip-update-firmware.sh -d -f` to enable fastboot flashing. Here is a [sample successful output](#debian-output).
...
During flashing, the terminal will fill with messages. If successful, you'll see C.H.I.P. run through a hardware test, with the answers being 'OK'.  If C.H.I.P. is 'OK', you can remove the jumper wire.
